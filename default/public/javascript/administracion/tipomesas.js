$(document).ready(function() {
	...
	$(document).on('click','.edit',function (e){  //para mostrar el formulario de edit
		configModalLoading();
		var idmesa=$(this).attr('data-id'); //obtiene el id de cada subprocuraduria
		$.get(url_base+"tipomesas/getNombretipo", {idtipo:idmesa}) // busca esa subproduradura
        .done(function(result){
        	$('#modal_cargando').modal('hide');
          	$('#editarsub').show('fast');
			$('.edit-sub').attr('action', url_base+"tipomesas/editar/"+idmesa); // le asigna la url correspondiente
			$('#inputmesa').val(result[0].name);  // muestra el nombre actual de la sub
        })
        .fail(function(err){
    	  $('#modal_cargando').modal('hide');
...
function eliminar(event){
    ...
    $.post(url_base+"tipomesas/eliminar", {'idtipo':id})
    .done(function(result){
